...
A quick-reference collection of bugs I've run into. The full diagnostic writeups live in the experiment logs.
...
2. Change from **Raspberry Pi** to **Blank for Linux**
   - Pick the **Linux** variant specifically. The generic "Blank" compiles for Windows and breaks things differently.
...
**Affects:** Every single program using OpenPLC Editor

**Symptom:** All timing measurements bunch up between 0–20ms with high variance and no pattern.

**Root cause:** The default OpenPLC scan/cycle time is ~20ms. Since the PLC logic only updates once per scan, every timing measurement gets quantized to 20ms chunks.
...
Doesn't need to be exact, within a few min is ok
